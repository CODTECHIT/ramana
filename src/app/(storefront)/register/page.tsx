...
export default function RegisterPage() {
  ...
  return (
    <main style={{ background: IVORY, minHeight: "80vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div className="w-full max-w-md p-8 my-10" style={{ border: `1px solid rgba(201,162,39,0.3)` }}>
        <h1 className="text-3xl font-normal text-center mb-2" style={{ fontFamily: SERIF, color: CHARCOAL }}>Create Account</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-xs uppercase tracking-wider mb-2" style={{ color: CHARCOAL, fontFamily: SANS }}>Full Name</label>
            <input 
              type="text" 
              required
              value={name}
        ...
        <div className="mt-8 text-center text-sm" style={{ color: SMOKE, fontFamily: SANS }}>
          ...
          <Link href={`/login?redirect=${encodeURIComponent(redirectUrl)}`} style={{ color: GOLD }}>
            Sign in
          </Link>
        </div>
      </div>
